...
export default function AvatarForm(props) {
    ...
    const [updateAvatar] = useMutation(UPDATE_AVATAR, {
        update(cache, { data: { updateAvatar } }) {
            // Actualiza la foto desde el cache del server de Apollo
            const { getUser } = cache.readQuery({
                // Obtiene los datos almacenados en la cache del server
                query: GET_USER,
                variables: {
            ...
            cache.writeQuery({
                // Función para reescribir la query a actualizar
                query: GET_USER,
                variables: {
                    username: auth.username,
                },
    ...
    return (
        <div className="avatar-form">
            <Button {...getRootProps()} loading={loading}>
                Sube tu foto <i className="file image icon" />
            </Button>
            <Button onClick={() => onDeleteAvatar()}>
                Elimina tu foto actual <i className="trash alternate icon" />
            </Button>
            <Button onClick={() => setShowModal(false)}>
                No hacer nada{" "}
                <i className="american sign language interpreting icon" />
            </Button>
            <input {...getInputProps()} />
        </div>
    );
